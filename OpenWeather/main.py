...
def build_query_param(city, lat, lon):
    ...
    if city and city.strip():  # Check if city is not empty and not just whitespace
        ...
        if isinstance(city, list):
            ...
            query_parts.extend(
                [c.strip() for c in city.split(",") if c.strip()]
...
def print_weather_result(data):
    ...
    locations = (
        data["location"] if isinstance(data["location"], list) else [data["location"]]
    )
    currents = (
        data["current"] if isinstance(data["current"], list) else [data["current"]]
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fetch current weather using weatherstack.com API"
    )
    parser.add_argument(
        "--city",
        type=str,
        help=(
            "City name (e.g. London or 'London,Paris,Berlin'). ",
            "Can be combined with coordinates.",
        ),
    )
    parser.add_argument(
        "--lat", type=float, help="Latitude. Can be combined with city."
    )
    parser.add_argument(
        "--lon", type=float, help="Longitude. Can be combined with city."
    )
